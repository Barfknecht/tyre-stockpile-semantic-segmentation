...
@click.option('--mask-root', type=click.Path(exists=True),
              default=dir + 'pngs')
@click.option('--weights-path', type=click.Path(exists=True),
              default='conversion/converted/dilation8_pascal_voc.npy')
...
def train(train_list_fname,
    ...
    train_desc = '{}-lr{:.0e}-bs{:03d}'.format(
        ...
        learning_rate,
        batch_size)
    checkpoints_folder = '/media/nicholas/Storage/' + train_desc
    try:
    ...
    train_basenames = [l.strip() for l in open(train_list_fname).readlines()][:100]
    val_basenames = [l.strip() for l in open(val_list_fname).readlines()]
    ...
    train_generator = datagen_train.flow_from_list(
            train_img_fnames,
            train_mask_fnames,
            shuffle=True,
    ...
    validation_generator = datagen_val.flow_from_list(
            val_img_fnames,
            val_mask_fnames,
            batch_size=8,
            img_target_size=(500, 500),
    ...
    model.fit_generator(train_generator,
        ...
        validation_steps=len(val_basenames)/batch_size,
        callbacks=[
            # model_checkpoint,
            # tensorboard_cback,
            # csv_log_cback,
            ...
            Metrics(val_img_fnames,val_mask_fnames)
        ]
    )
